...
3. On a positive intent match, `generate_pdf.get_info_and_generate_pdf()` runs: it fetches top exchanges, writes `top_stock_info.json`, and generates `market_report.pdf`.
...
- The function was evaluated by iterating through 5 distinct historical months and running them in the main function of `choose_top_stock.py` at the root directory of the project repository.
- The output results are checked against WFE website to verify the output printed on console are informatively correct.
- The result shows that all the tested case passed. The results in `2026/6`. `2026/5`, `2022/3`, `2018/1` can all be printed correctly. The result in `2015/12` prints the default 10 top stock exchanges from 2026/5 because WFE webpage only stores monthly report from 2018/1 and so on.
...
- Test case 3: Testing cases where APIs cannot be called successfully
  - Network disconnection was simulated to evaluate the system's offline fallback and error-handling mechanisms. As expected, if the root directory does not contain `market_report.pdf`, it will generate a PDF will lots of "N/A" fields (sample in `/generate_pdf3 Output/market_report_no_wifi`) at the root directory. Otherwise, it will not replace the existing PDF.
  - Output references: In `/generate_pdf3 Output` folder

### Functional Testing of `whatsapp_bot.py`
- The function was evaluated by sending messages to myself, and to my friends.
- The intention classifier works as intended. In my testing, all 8 text messages sent are correctly classified.
- The detection of new messages and the sending of the PDF file work smoothly under long sessions. It is observed that a new message take ~5 seconds to detect, and the PDF file takes ~8 seconds to generate and send.
- Output references: 
...
- The exchange-to-symbol mapping in `src/get_stock_info.py` is hardcoded and may not cover all future exchanges or symbol changes.
